...
class AudioLoader:
    ...
    def __init__(
        self,
        sources: Union[List[str], Dict] = None,
        ...
        if enhancement:
            self.clean_lists = util.read_sources(
            sources["clean"], relative_path=relative_path, ext=ext
            )
            self.noisy_lists = util.read_sources(
                sources["noisy"], relative_path=relative_path, ext=ext
            )
            self.audio_indices = [
            (src_idx, item_idx)
            for src_idx, src in enumerate(self.clean_lists)
    ...
    def __call__(
        ...
        if self.enhancement==False:
            ...
            if source_idx is not None and item_idx is not None:
                clean_info = self.clean_lists[source_idx][item_idx]
                noisy_info = self.noisy_lists[source_idx][item_idx]
            elif global_idx is not None:
                source_idx, item_idx = self.audio_indices[
                    global_idx % len(self.audio_indices)
                ]
                ...
                clean_info, source_idx, item_idx = util.choose_from_list_of_lists(
                    state, self.clean_lists, p=self.weights
                )
                noisy_info = self.noisy_lists[source_idx][item_idx]

            # 加载 clean & noisy
            clean_path, noisy_path = clean_info["path"], noisy_info["path"]
            
            if noisy_path == "none" or clean_path=="none":
                noisy_signal = AudioSignal.zeros(duration, sample_rate, num_channels)
                clean_signal = AudioSignal.zeros(duration, sample_rate, num_channels)
            noisy_signal = AudioSignal(noisy_path)
            clean_signal = AudioSignal(clean_path)
            if num_channels == 1:
                noisy_signal = noisy_signal.to_mono()
                clean_signal = clean_signal.to_mono()
            import torch
            if offset is None:
                signal = AudioSignal(
                                torch.cat([noisy_signal.audio_data, clean_signal.audio_data], dim=1),
                                sample_rate = clean_signal.sample_rate
                            )
                signal = signal.resample(sample_rate)
                T = int(duration * sample_rate)
                total_len = signal.audio_data.shape[-1]
                if total_len <= T:
                    audio_data = torch.nn.functional.pad(signal.audio_data, (0, T - total_len))
                else:
                # 随机选一个起始位置裁剪
                    if self.shuffle:
                        import torch
                        start = torch.randint(0, total_len - T + 1, (1,)).item()
                        audio_data = signal.audio_data[..., start:start + T]
                        signal = AudioSignal(audio_data,sample_rate)
                    else:
                        audio_data = signal.audio_data[..., :T]
                        signal = AudioSignal(audio_data,sample_rate)

            else:
                signal = AudioSignal(torch.cat([noisy_signal.audio_data, clean_signal.audio_data], dim=1),
                sample_rate = clean_signal.sample_rate,
                duration=duration,
                offset=offset)
            ...
            for k, v in clean_info.items():
                clean_signal.metadata[k] = v
            for k, v in noisy_info.items():
                noisy_signal.metadata[k] = v

            item = {
                "signal": signal,
                "source_idx": source_idx,
                "item_idx": item_idx,
                "source": str(self.sources["clean"][source_idx]),
