...
# Models
VODAC = argbind.bind(vodac.model.VODAC)
Discriminator = argbind.bind(vodac.model.Discriminator)
feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained("./hubert-base-ls960")
model = HubertModel.from_pretrained("./hubert-base-ls960").eval().cuda()
...
losses = argbind.bind_module(vodac.nn.loss, filter_fn=filter_fn)
...
def build_dataset(
    ...
    # cycles through them.
    datasets = []
    loader = AudioLoader(sources=folders,enhancement=True)
...
class State:
    ...
    distill_loss: losses.DistillLoss
...
def load(
    ...
    if resume:
        ...
        if (Path(kwargs["folder"]) / "vodac").exists():
            generator, g_extra = VODAC.load_from_folder(**kwargs)
        if (Path(kwargs["folder"]) / "discriminator").exists():
            discriminator, d_extra = Discriminator.load_from_folder(**kwargs)

    generator = VODAC() if generator is None else generator
...
def val_loop(batch, state, accel):
    ...
    signal = state.val_data.transform(
        batch["signal"], **batch["transform_args"]
    )
    out = state.generator(signal.audio_data[:,0:1,:], signal.sample_rate)
    if out["audio"].shape[-1] != signal.audio_data.shape[-1]:
        min_len = min(out["audio"].shape[-1], signal.audio_data.shape[-1])
        out["audio"] = out["audio"][..., :min_len]
        signal.audio_data = signal.audio_data[..., :min_len]
    clean_signal = AudioSignal(signal.audio_data[:,1:2,:], signal.sample_rate)
    recons = AudioSignal(out["audio"], signal.sample_rate)
    
    return {
        "loss": state.mel_loss(recons, clean_signal),
        "mel/loss": state.mel_loss(recons, clean_signal),
        "stft/loss": state.stft_loss(recons, clean_signal),
        "waveform/loss": state.waveform_loss(recons, clean_signal),
        "sisdr_loss": state.sisdr_loss(recons,clean_signal),
    }
...
def train_loop(state, batch, accel, lambdas):
    ...
    with torch.no_grad():
        ...
        clean_signal = AudioSignal(signal.audio_data[:,1:2,:], signal.sample_rate)
        noisy_signal = AudioSignal(signal.audio_data[:,0:1,:], signal.sample_rate)
    del signal
    torch.cuda.empty_cache()
    with torch.no_grad():
        with torch.cuda.amp.autocast():
            input_values = feature_extractor(clean_signal.audio_data.squeeze(0), sampling_rate=clean_signal.sample_rate, return_tensors="pt").input_values
            ouput_hu = model(input_values.squeeze().to(model.device), output_hidden_states=True)
            z_clean = torch.mean(torch.stack(ouput_hu.hidden_states), axis=0).transpose(-1,-2)
            #clean_features = rearrange(clean_features, 'b d t -> b t d')
            torch.cuda.empty_cache()

    # Generator forward
    with accel.autocast():
        out = state.generator(noisy_signal.audio_data, noisy_signal.sample_rate)
        recons = AudioSignal(out["audio"], noisy_signal.sample_rate)

        if out["audio"].shape[-1] != clean_signal.audio_data.shape[-1]:
            min_len = min(out["audio"].shape[-1], clean_signal.audio_data.shape[-1])
            out["audio"] = out["audio"][..., :min_len]
            clean_signal.audio_data = clean_signal.audio_data[..., :min_len]
        if z_clean.shape!=out["z"].shape:
            min_len = min(z_clean.shape[-1], out["z"].shape[-1])
    ...
    with accel.autocast():
        output["adv/disc_loss"] = state.gan_loss.discriminator_loss(recons, clean_signal,out["z"].transpose(-1,-2),z_clean.transpose(-1,-2))
    ...
    with accel.autocast():
        output["stft/loss"] = state.stft_loss(recons, clean_signal)
        output["mel/loss"] = state.mel_loss(recons, clean_signal)
        output["waveform/loss"] = state.waveform_loss(recons, clean_signal)
        (
            output["adv/gen_loss"],
            output["adv/feat_loss"],
        ) = state.gan_loss.generator_loss(recons, clean_signal, out["z"].transpose(-1,-2),z_clean.transpose(-1,-2))
    ...
    output["other/batch_size"] = noisy_signal.batch_size * accel.world_size
...
def save_samples(state, val_idx, writer):
    ...
    noisy_signal = AudioSignal(signal.audio_data[:,0:1,:], signal.sample_rate)
    out = state.generator(noisy_signal.audio_data, signal.sample_rate)
    recons = AudioSignal(out["audio"], signal.sample_rate)

    audio_dict = {"recons": recons}
    if state.tracker.step == 0:
        audio_dict["signal"] = noisy_signal
